    ...
    <div class="flex min-w-0 flex-col gap-1.5">
        ...
        trigger={
          <>
            <Avatar
              ...
              fallback={props.issue.assignee?.displayName ?? props.t("boc.jira.board.inspector.unassigned")}
            />
            <bdi dir="auto" class="truncate">
              {props.issue.assignee?.displayName ?? props.t("boc.jira.board.inspector.unassigned")}
            </bdi>
      ...
      >
        <div class="flex flex-col gap-2">
          ...
          />
          <div class="h-5 text-[12px] text-v2-text-text-muted" role="status">
            <Show when={picker.searching}>{props.t("boc.jira.collaboration.loading")}</Show>
          ...
          <div class="h-56 overflow-y-auto" inert={picker.searching}>
            <List
              ref={(value) => {
                list = value
              }}
            ...
            >
              {(user) => (
                <span class="flex min-w-0 items-center gap-2 text-[13px] leading-[var(--line-height-compact)]">
                  <Avatar size="small" src={user.avatarUrl} fallback={user.displayName} />
                  <span class="min-w-0">
                    <bdi dir="auto" class="block truncate">
                      {user.displayName}
                    </bdi>
                    <Show when={user.emailAddress}>
                      <bdi dir="auto" class="block truncate text-[12px] text-v2-text-text-muted">
                        {user.emailAddress}
                      </bdi>
                    </Show>
                  </span>
