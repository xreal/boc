...
story("shows Project setup and focuses search with Ctrl/Cmd+F", async ({ mount, page }) => {
  ...
  await expect(
    component.locator("article").filter({ hasText: "General" }).getByRole("button", { name: "Edit", exact: true }),
  ).toBeVisible()
  await expect(
...
story("keeps the project selector usable without overflow in narrow RTL", async ({ mount, page }) => {
  ...
  await expect(component.locator('[data-component="select-v2"][aria-label="Project"]')).toBeVisible()
  ...
  const dialog = page.getByRole("dialog")
  await dialog.getByText("More options", { exact: true }).click()
  for (const name of ["Activation in this file", "Agent mode"]) {
...
story("preserves invalid JSONC when reselecting the tab and guards Escape", async ({ mount, page }) => {
  const component = await mount("boc-controls--default")
  ...
  const dialog = page.getByRole("dialog")
  await dialog.getByRole("button", { name: "JSONC", exact: true }).click()
  const content = dialog.getByLabel("Definition · JSONC")
  await content.fill('{ "description": "unfinished')
  await dialog.getByRole("button", { name: "JSONC", exact: true }).click()
  await expect(content).toHaveValue('{ "description": "unfinished')
  await expect(dialog.getByRole("button", { name: "Settings", exact: true })).toBeDisabled()
  await expect(dialog.getByRole("button", { name: "Save changes" })).toBeDisabled()
  await page.keyboard.press("Escape")
  await expect(dialog.getByText("You have unsaved changes.")).toBeVisible()
  await dialog.getByRole("button", { name: "Keep editing" }).click()
  await expect(dialog.getByText("You have unsaved changes.")).toHaveCount(0)
  await expect(content).toHaveValue('{ "description": "unfinished')
  await content.fill('{ "description": "Repaired draft", "mode": "subagent" }')
  await dialog.getByRole("button", { name: "Save changes" }).click()
  await dialog.getByRole("button", { name: "Close", exact: true }).click()
  await expect(dialog).toHaveCount(0)
  await expect(edit).toBeFocused()
  await edit.click()
  await expect(dialog.getByLabel("Description")).toHaveValue("Repaired draft")
})

story("renames a new agent draft without losing fields or overwriting an existing agent", async ({ mount, page }) => {
  const component = await mount("boc-controls--default")
  await component.getByRole("button", { name: "Add agent", exact: true }).click()
  const dialog = page.getByRole("dialog")
  const id = dialog.getByLabel("Agent or server ID")
  await id.fill("release-draft")
  await dialog.getByLabel("Description").fill("Writes release notes")
  await dialog.getByLabel("Agent instructions").fill("Summarize the changes.")
  await id.fill("reviewer")
  await expect(dialog.getByRole("button", { name: "Save changes" })).toBeDisabled()
  await id.fill("release-notes")
  await expect(dialog.getByLabel("Description")).toHaveValue("Writes release notes")
  await expect(dialog.getByLabel("Agent instructions")).toHaveValue("Summarize the changes.")
  await dialog.getByRole("button", { name: "Save changes" }).click()
  await expect(id).toBeDisabled()
  await dialog.getByLabel("Description").fill("Writes concise release notes")
  await dialog.getByRole("button", { name: "Close", exact: true }).click()
  await dialog.getByRole("button", { name: "Keep editing" }).click()
  ...
  await component.getByRole("button", { name: "Config file", exact: true }).click()
  const content = dialog.getByLabel("Configuration content")
  await expect(content).toHaveValue(/"release-notes"/)
  await expect(content).toHaveValue(/Writes concise release notes/)
  await expect(content).toHaveValue(/"reviewer"/)
  await expect(content).not.toHaveValue(/"release-draft"/)
})

story("confirms agent removal and clears its default selection", async ({ mount, page }) => {
  const component = await mount("boc-controls--default")
  await component
    .locator("article")
    .filter({ hasText: "Reviewer" })
  ...
  await dialog.getByText("More options", { exact: true }).click()
  await dialog
    .getByRole("group", { name: "Agent mode", exact: true })
    .getByRole("button", { name: "Primary agent", exact: true })
    .click()
  await dialog.getByRole("button", { name: "Use as default agent" }).click()
  await dialog.getByRole("button", { name: "Save changes" }).click()
  await dialog.getByRole("button", { name: "Remove agent…", exact: true }).click()
  await expect(dialog.getByRole("button", { name: "Remove agent", exact: true })).toHaveAttribute(
    "data-variant",
    "danger",
  )
  await dialog.getByRole("button", { name: "Keep agent" }).click()
  await dialog.getByRole("button", { name: "Remove agent…", exact: true }).click()
  await dialog.getByRole("button", { name: "Remove agent", exact: true }).click()
  await expect(dialog).toHaveCount(0)
  await component.getByRole("button", { name: "Config file", exact: true }).click()
  await expect(dialog.getByLabel("Configuration content")).not.toHaveValue(/"reviewer"|"default_agent"/)
  await expect(dialog.getByLabel("Configuration content")).toHaveValue(/"planner"/)
})

story("keeps activation resets in details and restores the default", async ({ mount }) => {
  const component = await mount("boc-controls--default")
  const row = component.locator("article").filter({ hasText: "Review changes" })
  await row.locator('[data-slot="switch-control"]').click()
  await expect(row.getByRole("switch")).not.toBeChecked()
  await expect(row.getByRole("button", { name: "Use default activation" })).toHaveCount(0)
  await row.getByRole("button", { name: "Review changes", exact: true }).click()
  await row.getByRole("button", { name: "Use default activation" }).click()
  await expect(row.getByRole("switch")).toBeChecked()
  await expect(row.getByRole("button", { name: "Use default activation" })).toHaveCount(0)
})

story("preserves Markdown on outside click and discards only explicitly", async ({ mount, page }) => {
  const component = await mount("boc-controls--default")
  const edit = component
    .locator("article")
    .filter({ hasText: "Review changes" })
  ...
  await edit.click()
  const dialog = page.getByRole("dialog")
  await dialog.getByLabel("Instructions · Markdown").fill("Unsaved instructions")
  await page.mouse.click(5, 5)
  await expect(dialog.getByText("You have unsaved changes.")).toBeVisible()
  await dialog.getByRole("button", { name: "Keep editing" }).click()
  await expect(dialog.getByLabel("Instructions · Markdown")).toHaveValue("Unsaved instructions")
  await page.keyboard.press("Escape")
  await dialog.getByRole("button", { name: "Discard changes" }).click()
  await expect(dialog).toHaveCount(0)
  await edit.click()
  await expect(dialog.getByLabel("Instructions · Markdown")).toHaveValue(/Review the current diff/)
})

story("keeps nested model selection and dialog actions usable in a short narrow window", async ({ mount, page }) => {
  await page.setViewportSize({ width: 375, height: 600 })
  const component = await mount("boc-controls--dark")
  await component
    .locator("article")
    .filter({ hasText: "Reviewer" })
    .getByRole("button", { name: "Edit", exact: true })
  ...
  await expect(page.getByPlaceholder("Search models and providers")).toBeFocused()
  await page.keyboard.press("Escape")
  await expect(page.getByPlaceholder("Search models and providers")).toHaveCount(0)
  await expect(dialog).toBeVisible()
  await expect(dialog.getByRole("button", { name: "Model", exact: true })).toBeFocused()
  await dialog.getByLabel("Agent instructions").fill("A narrow viewport draft")
  await dialog.getByRole("button", { name: "Close", exact: true }).click()
  for (const name of ["Keep editing", "Discard changes", "Save changes"]) {
  ...
  await expect(dialog.getByLabel("Instructions · Markdown")).toBeVisible()
  await dialog.getByRole("button", { name: "Close", exact: true }).click()
  await expect(component.getByRole("button", { name: "Add skill", exact: true })).toBeFocused()
})

story("clears incompatible MCP fields and never revives a removed URL", async ({ mount, page }) => {
  const component = await mount("boc-controls--default")
  await component
    .locator("article")
    .filter({ hasText: "Documentation" })
    ...
    .click()
  const dialog = page.getByRole("dialog")
  const transport = dialog.getByRole("group", { name: "Connection type", exact: true })
  await transport.getByRole("button", { name: "Local · command" }).click()
  await expect(dialog.getByRole("button", { name: "Save changes" })).toBeDisabled()
  await dialog.getByLabel("Command and arguments · one per line").fill("bunx\nlocal-mcp")
  await dialog.getByRole("button", { name: "JSONC", exact: true }).click()
  const code = dialog.getByLabel("Definition · JSONC")
  await expect(code).not.toHaveValue(/"url"|"headers"|"oauth"/)
  await code.fill(
    '{ "type": "local", "command": ["bunx", "local-mcp"], "environment": { "LANG": "en" }, "cwd": "/workspace" }',
  )
  await dialog.getByRole("button", { name: "Settings", exact: true }).click()
  await transport.getByRole("button", { name: "Remote · URL" }).click()
  await expect(dialog.getByLabel("Server URL")).toHaveValue("")
  await expect(dialog.getByRole("button", { name: "Save changes" })).toBeDisabled()
  await dialog.getByLabel("Server URL").fill("https://replacement.example.com/mcp")
  await dialog.getByRole("button", { name: "JSONC", exact: true }).click()
  await expect(code).not.toHaveValue(/"command"|"environment"|"cwd"/)
  await expect(code).toHaveValue(/replacement.example.com/)
  await dialog.getByRole("button", { name: "Save changes" }).click()
  await expect(dialog.getByText("Saved. OpenCode reloads configuration automatically.")).toBeVisible()
})

story("shows actionable source errors and confirms deleting a skill", async ({ mount, page }) => {
  const component = await mount("boc-controls--default")
  await component
    .locator("article")
    .filter({ hasText: "Global review" })
    .getByRole("button", { name: "Edit", exact: true })
    .click()
  const dialog = page.getByRole("dialog")
  await expect(dialog.getByText("This source cannot be edited here.")).toBeVisible()
  await expect(dialog.getByRole("button", { name: "Save changes" })).toBeDisabled()
  await dialog.getByRole("button", { name: "Load again" }).click()
  await expect(dialog.getByText("This source cannot be edited here.")).toBeVisible()
  await dialog.getByRole("button", { name: "Close", exact: true }).click()
  await component
    .locator("article")
...
story("requires a fresh removal confirmation after changing the selected definition", async ({ mount, page }) => {
  const component = await mount("boc-controls--default")
  await component.getByRole("button", { name: "Config file", exact: true }).click()
  const dialog = page.getByRole("dialog")
  await dialog.getByRole("button", { name: "Common settings", exact: true }).click()
  ...
  await page.getByText("planner", { exact: true }).click()
  await dialog.getByRole("button", { name: "Remove agent…", exact: true }).click()
  await expect(dialog.getByRole("button", { name: "Remove agent", exact: true })).toBeVisible()
  await definition.click()
  await page.getByText("writer", { exact: true }).click()
  await expect(dialog.getByRole("button", { name: "Remove agent", exact: true })).toHaveCount(0)
  await expect(dialog.getByRole("button", { name: "Remove agent…", exact: true })).toBeVisible()
})
