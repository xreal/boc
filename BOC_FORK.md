...
| --- | --- | --- |
| Product identity | `boc` release channel, `Boc Beta` app identity, separate application data/service identity, red BergDev release icons, blue BergDev local-development icons, Boc branding in the empty file panel, desktop favicons and notifications, and updates from the public `boc-updates.bergdev.de` R2 endpoint. | `packages/desktop/icons/boc/`, `packages/desktop/icons/boc-dev/`, `packages/app/src/boc/brand-mark.tsx`, `packages/desktop/src/boc/` |
| Page search | Outside session routes, Cmd+F / Ctrl+F opens a page search bar with match highlighting, counts, next/previous navigation, Escape and focus restoration. Searches rendered text; collapsed/unloaded content, form fields and the search bar are excluded. Text changes refresh results while the bar is open. Session routes use upstream's message-data search and timeline reveal from `370b9965d3`, including matches outside the rendered viewport. The Boc command bridge unmounts page search on session routes to avoid competing shortcuts and highlights. | `packages/app/src/boc/commands.tsx`, `packages/app/src/boc/find.tsx`, `packages/app/src/boc/page-matches.ts`, `packages/app/src/boc/find.css` |
...
The exact API inventory assertion in `packages/client/test/promise.test.ts` includes `server.boc.worktree` alongside upstream groups. Keep this additive expectation when regenerating clients or syncing upstream.
