...
The portable private preview is `@bergflow/opencode` 0.2.0. Its browser-safe RPC export owns the shared schemas; neither Boc nor the TUI duplicates policy. Public plugin storage remains keyed by `bergflow`, and a protocol-owned process coordinator serializes project writes across separately evaluated Location module graphs. Successful persistence and failed application remain distinct. Unsupported agent/instruction policies are retained as unconfirmed, never simulated with text filtering.
