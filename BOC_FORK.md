...
| Bergflow Controls | Bundled passive project controls using the Jira/Deployments outer frame and compact fixed header, native server/project/worktree selection, search, category chips with inventory counts, responsive raised cards, neutral context borders, consistent input focus, disabled/applying/failed status badges, revision conflicts, reconnect refresh, and no automatic mutation retry. External plugins take precedence. Project-owned ambient `AGENTS.md` files, skills, observed tools, and configured MCP use supported public hooks; agents, global/unknown instructions, nested read-triggered instructions, and existing Session history remain read-only. Optional integrations require explicit activation. | `packages/boc/src/tools/bergflow/`, `packages/boc/vendor/`, `packages/app/src/boc/controls.ts`, `packages/core/src/boc/selection.ts`, `packages/plugin/src/boc/selection.ts` |
...
The Bergflow-backed screen, navigation entry, and session command are labeled **Project Controls**; plugin IDs, routes, storage, and diagnostic identity remain Bergflow-owned.

The host plugin loader supports opt-in fallback registration through `SdkPlugins.register(plugin, { fallback: true })`. External definitions with the same ID take precedence without duplicate activation. Ordered disable/enable operations and external options remain effective; fallback ID selectors can supply fallback options. Storage stays keyed by plugin ID. If an external source cannot be identified, unclaimed enabled fallbacks remain inactive with a diagnostic until that ambiguity is resolved; regular host plugins and successfully loaded external plugins remain available. This intentionally includes invalid configured local files or missing entrypoints. No package name or path is treated as a guessed plugin ID. Bergflow uses this seam only in the Boc channel. The content-addressed artifact in `packages/boc/vendor/` is adapted through the existing `PluginModule.Module` schema and `PluginPromise.fromPromise`; the portable package itself depends only on public plugin APIs. Controls use authenticated plugin RPC through the existing app server client, with no changes to desktop service ownership or server-version selection. Boc additionally exposes an optional versioned plugin-context selection facade backed by existing Location-scoped `PluginHooks`; it lists structured ambient instruction sources and permits exclusion before rendering without changing discovery producers, Session persistence, Protocol, or HTTP APIs.
...
| Host plugin fallback selection | `packages/core/src/plugin/sdk.ts`, `src/plugin/supervisor.ts`, `src/config/plugin/source.ts`, `test/plugin/supervisor-reload.test.ts` | Owner-approved Loader seam: opt-in fallbacks defer to external definitions, preserve configuration intent, and remain inactive on unidentified sources. Existing ordinary host precedence remains first-wins. Watch two-pass package activation, failed-reload retention, source scanning, and disable ordering. |
| Ambient instruction selection | `packages/core/src/instruction-discovery.ts`, `src/plugin/hooks.ts`, `src/plugin/host.ts`, `packages/plugin/src/effect/plugin.ts`, `src/promise/plugin.ts`, `src/promise/adapter.ts`, and the focused Core plugin fixture | Add one optional versioned plugin facade and filter the real structured discovery list before rendering. Candidate conversion and policy-neutral exclusion live under fork-owned `src/boc/` paths. Watch discovery availability, Location graph dependencies, Promise hook disposal, resolved checkout/global path classification, and recursive callback use. |
...
## Bergflow artifact and verification

The portable private preview is `@bergflow/opencode` 0.2.1. Its browser-safe RPC export owns the shared schemas; neither Boc nor the TUI duplicates policy. Public plugin storage remains keyed by `bergflow`, and a protocol-owned process coordinator serializes project writes across separately evaluated Location module graphs. Successful persistence and failed application remain distinct. Project-relative ambient instruction policy is applied through the structured Boc facade. Unsupported agent, original-server instruction, global/unknown instruction, and legacy absolute-path policies are retained as unconfirmed, never simulated with text filtering.

Tested original binaries are `0.0.0-beta-19192` and `0.0.0-beta-19234`; the release workflow pin remains `0.0.0-beta-19192`. No claim is made about arbitrary future beta APIs. The package is a private archive, not a published npm release. Native UI uses the existing English fallback i18n convention.

The source-control API audit reproduced an agent ordering limitation on both original binaries: config registers after external plugins; `AgentEditor.update` can recreate removed IDs. A late filter survives ordinary registry reloads but a later producer overrides it, including on subsequent reloads. There is no public final-selection phase, so agents remain read-only. Neither original binary exposes an instruction domain. Boc now filters structured ambient `InstructionDiscovery` files through the bounded selection facade. Read-triggered `SessionInstructions` remain a separate unmodified path, and already admitted instruction history is never retracted.

`packages/boc/scripts/bergflow/lifecycle-smoke.ts` accepts a Boc binary, compatible original binary, and incompatible original binary. It exercises real isolated service registrations in both startup orders, replacement and policy continuity from external plugin to bundle, and isolated fallback without touching the normal user's service. The portable repository's `scripts/artifact-smoke.ts` exercises packed installation, TUI discovery, optional integrations, multi-worktree revisions, and disabled/broken source selection. `packages/app/src/boc/bergflow.stories.tsx` mounts the production screen for visual and keyboard QA.
...
| `PluginHooks`, `InstructionDiscovery`, `FSUtil.resolve`/containment, and Promise plugin adaptation | Exposes and applies project-owned ambient instruction selection before rendering. | Hook lifecycle, discovery list/load coupling, unavailable-source behavior, Location graph dependencies, path canonicalization, and Promise callback conversion. |
