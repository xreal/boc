...
| Bergflow Controls | Bundled passive project controls with native server/project/worktree selection, search, truthful desired/effective state, revision conflicts, reconnect refresh, and no automatic mutation retry. External plugins take precedence. Agents/instructions are read-only; skills, observed tools and configured MCP use supported public hooks. Optional integrations require explicit activation. | `packages/boc/src/tools/bergflow/`, `packages/boc/vendor/`, `packages/app/src/boc/controls.ts` |
