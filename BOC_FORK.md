...
| Page search | Cmd+F / Ctrl+F opens a page search bar with match highlighting, counts, next/previous navigation, Escape and focus restoration. Searches rendered text, including loaded chat messages; collapsed/unloaded content, form fields and the search bar are excluded. Streaming text refreshes results while the bar is open. Uses the existing Boc command bridge and CSS Custom Highlights without modifying message markup. | `packages/app/src/boc/find.tsx`, `packages/app/src/boc/page-matches.ts`, `packages/app/src/boc/find.css` |
