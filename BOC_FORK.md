...
| --- | --- | --- |
| Product identity | `boc` release channel, `Boc Beta` app identity, separate application data/service identity, Boc icons, and updates from the public `boc-updates.bergdev.de` R2 endpoint. | `packages/desktop/icons/boc/`, `packages/desktop/src/boc/` |
...
| Release delivery | A Boc-only GitHub Actions workflow builds signed and notarized macOS packages plus unsigned Windows and Linux packages on official runners, keeps GitHub releases private, and activates public R2 updates only after validating every platform artifact. Immutable artifacts are versioned; updater manifests are uploaded last. | `packages/boc/scripts/release/`, `.github/workflows/boc-release.yml` |
