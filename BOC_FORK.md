...
| Jira board | Desktop-only Jira Cloud connection, encrypted token storage, read-only boards/sprints/issues, search and filters, saved preferences, cancellation, bounded rate-limit retries, and issue inspection. | `packages/boc/src/tools/jira/` |
...
| App routing, command, language, platform, server, dialog, and workspace hooks | Fork-owned bridges adapt Boc features to the app shell. | Context/provider placement, route parsing, command registration lifecycle, workspace creation/cache semantics. |
