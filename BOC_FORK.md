...
| Release delivery | A Boc-only GitHub Actions workflow builds signed desktop packages on official runners, keeps GitHub releases private, and activates public R2 updates only after validating every platform artifact. Immutable artifacts are versioned; updater manifests are uploaded last. | `packages/boc/scripts/release/`, `.github/workflows/boc-release.yml` |
