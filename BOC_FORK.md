...
The host plugin loader supports opt-in fallback registration through `SdkPlugins.register(plugin, { fallback: true })`. External definitions with the same ID take precedence without duplicate activation. Ordered disable/enable operations and external options remain effective; fallback ID selectors can supply fallback options. Storage stays keyed by plugin ID. If an external source cannot be identified, unclaimed enabled fallbacks remain inactive with a diagnostic until that ambiguity is resolved; regular host plugins and successfully loaded external plugins remain available. This intentionally includes invalid configured local files or missing entrypoints. No package name or path is treated as a guessed plugin ID. The seam is implemented and tested, but Bergflow is not yet bundled.
